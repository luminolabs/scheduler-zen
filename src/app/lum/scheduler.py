...
from app.core.utils import (
    ...
    setup_logger, JOB_STATUS_WAIT_FOR_VM, AsyncRetry
)
...
retry_config_tx_receipt = AsyncRetry(
    attempts=60,
    delay=0.1,
    deadline=60,
...
class LUMReceiptProcessor:
    """
    Handles processing of transaction receipts and LUM ID extraction.
    Extracted for better testability.
    """
    def __init__(self, job_manager_client: JobManagerClient):
        self.job_manager_client = job_manager_client

    @retry_config_tx_receipt
    async def get_tx_receipt(self, tx_hash: str) -> Optional[Tuple[str, Dict[str, Any]]]:
        """
        ...
        Returns:
            Optional[Tuple[str, Dict[str, Any]]]: Transaction hash and receipt if found, None otherwise
        """
    ...
    def get_lum_id_from_receipt(self, receipt: Dict[str, Any]) -> Optional[int]:
        """
...
class LUMJobStatusUpdater:
    """
    Handles job status updates.
    Extracted for better testability.
    """
    def __init__(self, job_manager_client: JobManagerClient, db: Database):
        self.job_manager_client = job_manager_client
        self.db = db

    async def update_job_status_single(self, job: Dict[str, Any]) -> None:
        """
...
class Scheduler:
    ...
    Now uses composition with specialized helper classes for better testability.
    """
    def __init__(self, db: Database, job_manager_client: JobManagerClient):
        ...
        Args:
            ...
            job_manager_client (JobManagerClient): The JobManagerClient for blockchain interactions.
        """
        self.db = db
        self.job_manager_client = job_manager_client
        self.running = False
        self.receipt_processor = LUMReceiptProcessor(job_manager_client)
        self.status_updater = LUMJobStatusUpdater(job_manager_client, db)
    ...
    async def run_cycle(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error in LUM scheduler cycle, "
    ...
    async def _update_job_statuses(self) -> None:
        ...
        await asyncio.gather(*[
            self.status_updater.update_job_status_single(job)
    ...
    async def _process_pending_receipts(self) -> None:
        ...
        receipt_tasks = [
            self.receipt_processor.get_tx_receipt(job['lum']['tx_hash'])
            for job in pending_jobs
        ]
        results = await asyncio.gather(*receipt_tasks)
        ...
        for result in results:
            ...
            # Extract tx_hash and receipt
            tx_hash, receipt = result
            # Get the job from the mapping
            job = tx_hash_to_job[tx_hash]
            # Extract the LUM ID from the receipt
            lum_id = self.receipt_processor.get_lum_id_from_receipt(receipt)
            if lum_id:
                async with self.db.transaction() as conn:
                    await self.db.update_job_lum(
                        conn, job['job_id'], job['user_id'], lum_id=lum_id
                    )
